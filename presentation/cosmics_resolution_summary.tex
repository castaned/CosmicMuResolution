...
\newcommand{\tightLooseTable}{
  \begin{tabular}{@{}lll@{}}
    \toprule
    Quantity & Tag-quality only & Resolution category \\
    \midrule
    Tag/probe pairing & Upstream ntuplizer & Same upstream pairing \\
    Tag quality definition & applied upstream & kept for the tag leg \\
    Probe quality definition & looser than tag & matched to the tag definition \\
    $p_{T}^{err}/p_{T}$ (DGL) & tag only & $< 0.3$ on tag and probe \\
    $\chi^{2}$ (DGL) & tag only & $< 5$ on tag and probe \\
    MuonHits (DGL) & tag only & $\geq 13$ on tag and probe \\
    ValidStripHits (DGL) & tag only & $\geq 6$ on tag and probe \\
    $p_{T}^{err}/p_{T}$ (DSA) & tag only & $< 0.2$ on tag and probe \\
    $\chi^{2}$ (DSA) & tag only & $< 5$ on tag and probe \\
    ValidMuonDTHits (DSA) & tag only & $\geq 31$ on tag and probe \\
    \bottomrule
  \end{tabular}
}
...
\begin{frame}{Scenario Definitions}
  ...
  \begin{itemize}
    ...
    \item We start from a quality definition for the tag leg, because the tag is used as the reference object of the pair.

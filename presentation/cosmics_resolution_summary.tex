...
\begin{frame}{How To Read The Residual-Fit Panels}
  \threebulletsummary
    {Each $p_{T}$ bin contains one residual histogram; the colored curves are fit functions over the same distribution, not separate event samples.}
    {The single-Gaussian fit is the baseline model used to extract the reported mean and resolution.}
    {The double-Gaussian fit is kept only as a cross-check for non-Gaussian tails; the final DGL results shown in this talk use the single-Gaussian summary.}
\end{frame}
